...
@section('content')
    <div class="container mt-3">
        <h2>Обновление бренда</h2>
        @if (session('success'))
            <div class="mx-5 mt-2 alert alert-info w-25">
                {{ session('success') }}
            </div>
        @endif
        <form method="POST" action="{{ route('brand_update_valid') }}">
            @csrf
            <div class="row mb-3">
                <label for="title_brand" class="col-md-4 col-form-label text-md-end">Название</label>
                <div class="col-md-6">
                    <input type="hidden" value="{{$brand->id}}" name="id">
                    <input id="title_brand" type="text" class="form-control" name="title_brand" value="{{$brand->title_brand}}" required>
                    @error('title_brand')
